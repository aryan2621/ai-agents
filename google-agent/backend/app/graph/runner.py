...
def build_react_graph(
    ...
    tools = get_tools_for_agent(
        agent_name, google, workspace_context, tavily_api_key=tavily_api_key
    )
    if not tools:
        return None
    ...
    prompt = build_system_prompt(get_agent_prompt(agent_name, google), workspace_context)
...
async def stream_agent(
    ...
    graph = build_react_graph(
        agent_name, google, settings, ctx, tavily_api_key=tavily_api_key
    )

    if graph is None:
        async for chunk in _stream_simple(
            agent_name, message, history, google, settings, ctx
        ):
            yield chunk
        return
