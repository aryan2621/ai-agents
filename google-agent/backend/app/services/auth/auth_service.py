...
from app.models.auth import GoogleUserResponse, PermissionStatusResponse
from app.services.auth.scope_registry import (
    build_permission_statuses,
    effective_granted_scopes,
)
from app.services.auth.token_crypto import decrypt_token, encrypt_token
    ...
    if token is None:
        session.add(
            OAuthToken(
                ...
                google_access_token=encrypt_token(creds.google_access_token),
        ...
        token.google_access_token = encrypt_token(creds.google_access_token)
...
async def get_credentials_by_access_token(
    ...
    return StoredCredentials(
        ...
        google_access_token=decrypt_token(token.google_access_token),
...
async def update_google_access_token(
    ...
    if row is None:
        return None
    token, user = row
    token.google_access_token = encrypt_token(google_access_token)
...
def to_user_response(creds: StoredCredentials) -> GoogleUserResponse:
