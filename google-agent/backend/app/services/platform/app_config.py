...
def resolve_google_credentials() -> tuple[str, str]:
    config = load_app_config()
    settings = get_settings()
    client_id = (config.get("GOOGLE_CLIENT_ID") or settings.google_client_id or "").strip()
    client_secret = (config.get("GOOGLE_CLIENT_SECRET") or settings.google_client_secret or "").strip()
