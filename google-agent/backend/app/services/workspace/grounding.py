...
MUTATION_STATUSES = frozenset(
    {"created", "updated", "deleted", "appended", "sent", "trashed"}
)
MUTATION_CLAIM_RE = re.compile(
    r"(?i)\b(?:"
    r"(?:i|we)\s+(?:have\s+|just\s+|successfully\s+)?"
    r"(?:created|sent|emailed|updated|deleted|trashed|scheduled|appended|booked)"
    r"|successfully\s+(?:created|sent|emailed|updated|deleted|trashed|scheduled|appended|booked)"
...
def ground_assistant_text(
    ...
    *,
    ...
    if not payloads:
        if original and QUESTION_RE.search(original) and not MUTATION_CLAIM_RE.search(original):
            return original
