...
from app.services.platform.llm_keys import apply_keys_to_llm_settings, llm_provider_configured
...
from app.services.platform.datetime_display import humanize_iso_datetimes
from app.services.google_clients import GoogleClients
from app.services.auth.google_oauth import get_valid_credentials
...
def _permission_error_payload(agent: str, scope: str) -> PermissionErrorPayload:
    ...
    return {
        "error": f"{label} permission was not granted in Google. Re-authenticate to grant access.",
...
async def _stream_chat(
    ...
    google = GoogleClients(creds)
    display_tz = google.get_calendar_timezone()
