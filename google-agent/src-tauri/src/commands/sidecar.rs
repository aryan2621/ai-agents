...
async fn backend_has_ollama_stack() -> bool {
    ...
    capabilities_include(&body, "ollama")
        ...
        && (build_id.is_empty() || build_id == "ollama-only-v10")
}
...
pub async fn start_python_backend_internal(app: &tauri::AppHandle) {
    if backend_is_healthy().await {
        if backend_is_current().await {
            eprintln!(
                "[google-agent] Backend already running on http://127.0.0.1:8000 — skipping sidecar spawn"
            );
        ...
        eprintln!(
            "[google-agent] Stale backend on http://127.0.0.1:8000 (missing Ollama-only stack) — restarting sidecar. Run: npm run build:sidecar"
        );
