...
function buildIssuesFromChecks(checks: Record<string, string>): ReadinessIssue[] {
  ...
  if (checks.llm === 'error') {
    issues.push({
      ...
      remediation: 'Start Ollama and pull a model (ollama pull qwen2.5:7b).',
    })
  }
  return issues
}
...
export async function fetchMe(accessToken?: string): Promise<GoogleUser> {
  ...
  return ky.get(`${API_BASE_URL}/auth/me`, { headers }).json<GoogleUser>()
}
...
function parseSettings(raw: Record<string, unknown>): Settings {
  ...
  return {
    defaultModel: String(raw.defaultModel ?? raw.default_model ?? 'qwen2.5:7b'),
