...
from app.services.auth_service import delete_user_session, get_user_by_access_token, to_user_response
...
from app.services import oauth_pending
...
<body>
  <div class="card">
    ...
    <p>Click below to open GitHub sign-in. This tab will close when you finish.</p>
    <button type="button" id="signin-btn">Continue with GitHub</button>
  </div>
  <script>
    (function() {{
...
async def github_auth_url():
    ...
    if not client_id:
        raise HTTPException(
            status_code=500,
            detail="GitHub OAuth is not configured. Add credentials in Settings → GitHub OAuth.",
        )
    state = secrets.token_urlsafe(24)
    oauth_pending.mark_pending(state)
    return {
        "url": build_github_auth_url(client_id, state),
        "state": state,
    }


@router.get("/github/status/{state}", response_model=GitHubUserResponse)
async def github_auth_status(state: str):
...
@router.get("/github/begin")
async def github_auth_begin(state: str):
    """Opens GitHub OAuth in a popup so the callback window can close itself."""
    ...
    auth_url = build_github_auth_url(client_id, state)
    return HTMLResponse(_popup_launcher_html(auth_url))


@router.get("/github/callback")
async def github_browser_callback(
    code: str | None = None,
    error: str | None = None,
    state: str | None = None,
    ...
    if error:
        ...
        return HTMLResponse(
            _callback_html(
                "Sign-in failed",
                "Return to GitHub Agent and try again.",
            ),
            status_code=400,
        )
    ...
    try:
        user = await exchange_code(session, code, GITHUB_OAUTH_REDIRECT_URI)
    ...
    deep_link = f"github-agent://oauth/callback?state={quote(state)}"
    return HTMLResponse(
        _callback_html(
            "Signed in successfully",
            "Returning to GitHub Agent…",
            deep_link=deep_link,
        )
    )
...
async def get_me(
    ...
    user, token = row
    if not (user.picture or "").strip() and token.github_access_token:
        picture = await fetch_profile_picture(decrypt_token(token.github_access_token))
        if picture:
