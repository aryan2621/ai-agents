...
async function pollAuthStatus(state: string): Promise<GitHubUser | null> {
...
export function useGitHubAuth() {
  ...
  const signInWithGitHub = useCallback(
    async (options?: SignInOptions) => {
      ...
      const state = await invoke<string>('start_github_auth')
      toast.message('Click “Continue with GitHub” in the browser tab to sign in.')

      const user = await pollAuthStatus(state)
      if (!user) {
        throw new Error('Timed out waiting for GitHub sign-in. Close the browser tab and try again.')
      }
