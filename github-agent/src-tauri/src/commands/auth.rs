...
const BACKEND_AUTH_URL: &str = "http://127.0.0.1:8000/auth/github/url";
const OAUTH_BEGIN_URL: &str = "http://127.0.0.1:8000/auth/github/begin";
const OAUTH_REDIRECT_URI: &str = "http://127.0.0.1:8000/auth/github/callback";
...
async fn fetch_github_auth_start(client: &reqwest::Client) -> Result<AuthStart, String> {
...
pub async fn start_github_auth(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let auth_start = fetch_github_auth_start(&client).await?;
...
pub async fn handle_oauth_callback(code: String) -> Result<serde_json::Value, String> {
    let client = reqwest::Client::new();
    let response = client
        .post("http://127.0.0.1:8000/auth/github/callback")
